...
const SUGGESTED_PROMPTS = [
  {
    ...
    prompt: "Do we have any data on Operation Killshot preorders?",
  },
  {
    icon: Database,
    title: "Find Multiple Sources",
    prompt: "Find revenue data and player behavior data for Battle Royale games",
  },
  {
    icon: Gamepad2,
    title: "List Our Games",
    prompt: "What games does our company produce?",
  },
  {
    icon: Shield,
    title: "Find Policy",
    prompt: "Show me data governance policies for player data",
  },
  {
    icon: User,
    title: "Find Person",
    prompt: "Who is Sarah Chen and what does she work on?",
  },
  {
    icon: Users,
    title: "Find Team",
    prompt: "Show me the Gaming Analytics team members",
  },
  {
    icon: Wrench,
    title: "Find Tool",
    prompt: "What analytics tools do we have for player segmentation?",
  },
  {
    icon: Folder,
    title: "Find Collection",
    prompt: "Show me collections related to player retention analysis",
  },
  {
    icon: Key,
    title: "Request Access",
    prompt: "How do I get access to Cosmic Carnival event data?",
  },
  {
    icon: TrendingUp,
    title: "Data Quality",
    prompt: "Show me the highest trust score datasets for RPG analysis",
  },
  {
    icon: Building,
    title: "Find by Studio",
    prompt: "What data sources are available from Shadow Labs studio?",
  },
  {
    icon: Scale,
    title: "Governance Info",
    prompt: "What are the compliance requirements for revenue data?",
  },
];
...
export function ChatInterface() {
  ...
  // Smart grid layout logic based on number of cards
  const getGridLayout = (count: number) => {
    if (count === 1) return "grid-cols-1 max-w-md mx-auto";
    if (count === 2) return "grid-cols-1 md:grid-cols-2 max-w-4xl mx-auto";
    if (count === 3) return "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 max-w-5xl mx-auto";
    if (count === 4) return "grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 max-w-6xl mx-auto";
    if (count <= 6) return "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto";
    if (count <= 9) return "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 max-w-6xl mx-auto";
    return "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 max-w-6xl mx-auto";
  };
  ...
  const renderAIMessageParts = (parts: Array<Record<string, unknown>>) => {
    ...
    // Render cards FIRST (above text)
    // If we have both people and data source parts, render them side by side with proper alignment
    if (peopleParts.length > 0 && dataSourceParts.length > 0) {
      renderedParts.push(
        <div key="side-by-side-cards" className="my-8">
          <div className="bg-gradient-to-r from-blue-50/30 to-purple-50/30 rounded-xl p-6 border border-gray-100">
    ...
    const renderMessagePart = (part: Record<string, unknown>, index: number) => {
      switch (part.type) {
        ...
        case 'data-source-grid':
          ...
          return (
            <div key={index} className={`grid gap-2 ${gridLayout}`}>
              {sources.map((source: Record<string, unknown>, sourceIndex: number) => {
                const IconComponent = getTypeIcon(String(source.type || 'dataset'));
                return (
                  <Card key={sourceIndex} className="transition-all hover:shadow-lg border-2 hover:border-primary/20 h-full flex flex-col">
                    ...
                    <CardContent className="pt-0 px-4 pb-4 space-y-1 flex-grow flex flex-col justify-between">
                      ...
                      <div className="grid grid-cols-3 gap-1">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => window.open("https://alation.com", "_blank")}
                          className="text-xs"
                        >
        ...
        case 'data-people-grid':
          ...
          return (
            <div key={index} className={`grid gap-4 ${peopleGridLayout}`}>
              {people.map((person: Record<string, unknown>, personIndex: number) => (
                <Card key={personIndex} className="transition-all hover:shadow-lg border-2 hover:border-primary/20 h-full flex flex-col">
                  <CardHeader className="pb-3">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <Button
                          ...
                          variant="outline"
                          onClick={() => {
                            window.open(`mailto:${person.email}?subject=Data Marketplace Inquiry`, '_blank');
              ...
              <div className="text-center space-y-6">
                ...
                <div>
                  ...
                  <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                    Discover data sources, understand governance, request access, and build collections for your data projects
                  </p>
                </div>
              </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-background/95 backdrop-blur-sm z-50">
        <div className="max-w-6xl mx-auto px-4 py-3">
          ...
          <p className="text-xs text-muted-foreground text-center mt-3">
            Data Marketplace can make mistakes. Check important info.
          </p>
        </div>
      </div>
    </div>
