...
import { getMockInsightById } from '@/lib/mock-data';
...
export async function GET(
  ...
  try {
    ...
    if (shouldUseMockData()) {
      const insight = getMockInsightById(id);
      if (!insight) {
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Failed to fetch insight' },
      { status: 500 }
    );
  }
}
