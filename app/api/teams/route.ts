...
export async function GET(request: NextRequest) {
  try {
    ...
    if (shouldUseMockData()) {
      const teams = getMockTeams({ search: search || undefined, type: type || undefined, department: department || undefined, includeMembers });
...
export async function POST(request: NextRequest) {
  try {
    if (shouldUseMockData()) {
      return NextResponse.json(
        { success: false, error: 'Team creation is disabled in mock mode (demo only).' },
        { status: 400 }
      );
    }
