...
export async function GET(
  ...
  try {
    ...
    if (shouldUseMockData()) {
      const result = getMockTeamMembers(teamId);
      const members = (result as Array<Record<string, unknown>>).map((member: Record<string, unknown>) => ({
...
export async function POST(
  ...
  try {
    if (shouldUseMockData()) {
      return NextResponse.json(
        { success: false, error: 'Team membership changes are disabled in mock mode (demo only).' },
        { status: 400 }
      );
    }
