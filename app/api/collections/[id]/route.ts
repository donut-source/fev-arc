...
import { getMockCollectionById } from '@/lib/mock-data';
...
export async function GET(
  ...
  try {
    ...
    if (shouldUseMockData()) {
      const result = getMockCollectionById(id);
      if (!result) {
      ...
      const dataSources = result.data_sources.map((row) => ({
        ...row,
        tags: row.tags || [],
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          owner_name: result.collection.owner_name,
          visibility: result.collection.visibility,
          is_published: result.collection.is_published,
